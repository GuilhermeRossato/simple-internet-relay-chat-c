...
int irc_on_message_received(char * origin, char * message) {
	...
	if (!irc_check_user_by_origin(origin)) {
		success = irc_create_user(origin);
		if (!success) {
			return 0;
		}
	}
	...
	if (message[0] != '\0') {
		return irc_broadcast_message(origin, message);
	} else if (irc_is_command(message, "join")) {
		return irc_join_channel(origin, irc_get_command_parameter(message, "join"));
	} else if (irc_is_command(message, "create")) {
		return irc_create_channel(origin, irc_get_command_parameter(message, "create"));
	} else if (irc_is_command(message, "leave")) {
		return irc_leave_channel(origin, irc_get_command_parameter(message, "leave"));
	} else if (irc_is_command(message, "exit")) {
		return irc_exit_user(origin);
	} else {
		return irc_error_object_not_found("specified command");
	}
}

int irc_join_channel(char * origin, char * channel_name) {
...
int irc_exit_channel(char * origin, char * channel_name) {
